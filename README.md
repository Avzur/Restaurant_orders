...
<br><br>**שלב 1:** מיפוי הישויות (Entities), סריקת רשימת הטבלאות הפיזיות בבסיס הנתונים שהתקבל. כל טבלה עצמאית שמייצגת עצם בעולם האמיתי (כגון: tourist, attraction, coupon) מתורגמת למלבן של ישות בתרשים ה-ERD.
<br><br>
* הגדרת תכונות (Attributes) ומפתחותעבור כל טבלה, חקירת העמודות וסוגי הנתונים שלהן.עמודות רגילות מתורגמות לשדות (עיגולים) המחוברים לישות. עמודה המוגדרת כמפתח ראשי (Primary Key) מסומנת בקו תחתון בתוך העיגול שלה בתרשים הERD.
<br><br>
